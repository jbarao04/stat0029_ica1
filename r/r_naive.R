...
# mm_naive.R
#
# Baseline naive triple-loop matrix multiplication for 1024x1024.
# Usage from cmd:
#   Rscript mm_naive.R [reps] [csv_path]
# Example:
#   Rscript mm_naive.R 5 results_r.csv
...
append_results_csv <- function(csv_path, rows) {
  # rows is a data.frame with columns: language, algorithm, n, rep, time_s
...
matmul_naive <- function(A, B) {
  n <- nrow(A); m <- ncol(B); p <- ncol(A)
...
main <- function() {
  ...
  cat(sprintf("Running %s algorithm in %s on %dx%d matrices (%d reps, excluding warm-up)...\n",
